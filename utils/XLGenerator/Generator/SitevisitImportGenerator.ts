...
export async function importSiteVisitWithAllColumns(): Promise<string> {
  ...
  const filePath = await utils.generateExcelFile("SiteVisitImport", properties.ImportLocation);
  ...
  for (let i = 1; i <= fileCount; i++) {
    ...
    const leadId = (await new LeadAPIUtils().createLeadWithDetails(phone, email)).sell_do_lead_id;
