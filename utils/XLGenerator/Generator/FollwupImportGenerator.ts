import { XLUtils } from "../XLUtils.ts";
import { Utils, AheadOf } from "../../PlaywrightTestUtils.ts";
...
import { LeadAPIUtils } from "../../APIUtils/LeadAPIUtils.ts";

export async function importFollowupWithAllColumns(): Promise<string> {
  const sheetName = "sheet 1";
    const utils = new Utils();
    const filePath = await utils.generateExcelFile("FolloupImport", properties.ImportLocation, sheetName);
    const xl = new XLUtils(filePath);
    xl.createFileWithHeader(sheetName, CreateHeaderForFollowupImport_allFields);

    const fileCount = parseInt(properties.ImportCount.toString());
    for (let i = 1; i <= fileCount; i++) {
      ...
      const leadId = (await new LeadAPIUtils().createLeadWithDetails(
        phone,
        email
      )).sell_do_lead_id;
